...
def Interpret(PGN:str, display=True) -> tuple[list, list, list]:
    """
    ...
    for turns in pgn:
        for turn in turns:
            ...
            if turn[0].islower():
                ...
                if turn[0] != 'O':
                    piece = letter_to_piece_number(turn[0]) * game.player_color
                    if len(turn) == 3:
                        new_x = letter_to_index(turn[1])
                        new_y = 8 - int(turn[2])

                        for piece in (game.white_pieces if game.player_color == 1 else game.black_pieces):
                            if Chess.coords_2D_to_1D(new_x, new_y) in game.get_moves(piece.pos, piece.color):
            ...
            #if game.player_color == -1:
            #    boards.append(np.flipud(game.board))
